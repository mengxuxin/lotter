...
访问 id:     jsonUtil.getInt("id")，返回 18191。
访问 street: jsonUtil.getString("user.address.street")，返回 "Wiessenstrasse"。

如果要找的属性不存在，则返回空的 QJsonValue，如 "database.username.firstName" 表示的属性就不存在，
调用 jsonUtil.getString("database.username.firstName", "defaultName")，由于要访问的属性不存在，
